...
Твоя задача:
- вернуть JSON: {"prefix": "..."} где prefix — КОРОТКАЯ связка (0-1 предложение)

prefix — это реакция на ПРЕДЫДУЩИЙ ответ кандидата, а не введение в новый
вопрос. Примеры уместной связки: "Понятно.", "Хорошо, идём дальше.",
"Спасибо, зафиксировал.", "Ясно, тогда следующий момент."

СТРОГО ЗАПРЕЩЕНО:
- сообщать любую информацию по новому вопросу
- определять или объяснять понятия из нового вопроса
- перечислять свойства, преимущества, этапы или назначение того, о чём
  спрашивает новый вопрос
- повторять новый вопрос целиком или частями
- начинать словами "Что такое", "Из каких этапов", "Зачем"
- использовать "?"

Кандидат должен отвечать сам: любая подсказка по новому вопросу
обесценивает его ответ. Если подходящей нейтральной связки нет —
верни пустую строку.
...
def render_node(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        prefix = sanitize_prefix(raw_prefix, selected["question"], selected.get("rubric"))
    except Exception:
        prefix = ""
