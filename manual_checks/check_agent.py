...
def main():
    ...
    # ------------------------------------------------------------
    # SCENARIO 3: ответ УШЁЛ НЕ ТУДА (дрейф).
    # Ответ намеренно не про ML вообще, чтобы проверка не зависела от того,
    # какой вопрос выдал ретривал: ответ про ML был бы не по теме только
    # при определённом вопросе.
    ...
    print(f"\n[S3] вопрос на этом ходу: {s2.get('last_question_text')}")

    s3 = run_turn(
        thread_config=config,
        user_text=(
            "Вчера ходил на футбол, наши выиграли три-один. "
            "Погода, правда, подвела: весь второй тайм лил дождь."
        ),
    )
